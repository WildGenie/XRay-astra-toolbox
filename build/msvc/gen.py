...
def create_mex_project(name, uuid11, uuid09):
  return {
      "type": vcppguid,
      "name": name,
      "file11": f"matlab\\mex\\{name}_vc11.vcxproj",
      "file09": "matlab\\mex\\" + name + "_vc09.vcproj",
      "uuid11": uuid11,
      "uuid09": uuid09,
      "files": [],
  }
...
P_astra["filters"] = {
    "Algorithms": ["262b0d17-774a-4cb1-b51a-b358d2d02791"],
    "Data Structures": ["76d6d672-670b-4454-b3ab-10dc8f9b8710"],
    "Projectors": ["77a581a9-60da-4265-97c0-80cdf97408c0"],
    "CUDA": ["c1af0e56-5fcc-4e75-b5db-88eeb4148185"],
    "Global &amp; Other": ["72fbe846-10ef-4c52-88df-13bd66c4cbfc"],
    "Geometries": ["7ef37c12-c98c-4dd6-938d-12f49279eae0"],
    "CUDA\\cuda source": [
...
bom = "\xef\xbb\xbf" if six.PY2 else codecs.BOM_UTF8.decode("utf-8")
...
class Configuration:
  ...
  def config(self):
    ...
    return n
  def platform(self):
    return "x64" if self.x64 else "Win32" 
  def name(self):
    n = self.config()
    n += "|"
    n += self.platform()
...
def write_project11_start(P, F):
  ...
  for c in configs:
    ...
    print(f'      <Configuration>{c.config()}</Configuration>', file=F)
    print(f'      <Platform>{c.platform()}</Platform>', file=F)
...
def write_project11_end(P, F):
  if l := [f for f in P["files"] if len(f) > 4 and f[-4:] == ".cpp"]:
    print('  <ItemGroup>', file=F)
    for f in l:
      if ("cuda" in f) or ("Cuda" in f):
        ...
        print('    <ClCompile Include="' + f + '" />', file=F)
    print('  </ItemGroup>', file=F)
  if l := [f for f in P["files"] if len(f) > 2 and f[-2:] == ".h"]:
    print('  <ItemGroup>', file=F)
    for f in l:
      print('    <ClInclude Include="' + f + '" />', file=F)
    print('  </ItemGroup>', file=F)
  if l := [f for f in P["files"] if len(f) > 3 and f[-3:] == ".cu"]:
    print('  <ItemGroup>', file=F)
    for f in l:
    ...
    print('  </ItemGroup>', file=F)
  if l := [f for f in P["files"] if len(f) > 4 and f[-4:] == ".inl"]:
    print('  <ItemGroup>', file=F)
    for f in l:
...
def write_main_project11():
  ...
  with open(P["file11"], "w") as F:
    ...
    for c in configs:
      ...
      print(
          f'    <OutDir>$(SolutionDir)bin\\$(Platform)\\{c.config()}' +
          '\\</OutDir>',
          file=F,
      )
      print('    <IntDir>$(OutDir)obj\\</IntDir>', file=F)
      print('    <TargetExt>.dll</TargetExt>', file=F)
      print(f'    <TargetName>{c.target()}</TargetName>', file=F)
    ...
    for c in configs:
      ...
      print(
          f'      <OutputFile>bin\\{c.platform()}\\{c.config()}\\{c.target()}'
          + '.dll</OutputFile>',
          file=F,
      )
      if c.cuda:
        print('      <AdditionalDependencies>cudart.lib;cufft.lib;%(AdditionalDependencies)</AdditionalDependencies>', file=F)
      l = '      <AdditionalLibraryDirectories>' + ('lib\\x64'
                                                    if c.x64 else 'lib\\win32')
...
def write_mex_project11(P):
  with open("matlab/mex/" + P["name"] + "_vc11.vcxproj", "w") as F:
    write_project11_start(P, F)
    print('  <PropertyGroup>', file=F)
...
#      print('      <OptimizeReferences>true</OptimizeReferences>', file=F)
      ...
      l = '      <AdditionalLibraryDirectories>' + (
          '..\\..\\lib\\x64\\;..\\..\\bin\\x64\\'
          if c.x64 else '..\\..\\lib\\win32\\;..\\..\\bin\\win32\\')
      l += c.config()
      if c.x64:
...
def write_main_filters11():
  P = P_astra
  with open(P["name"] + ".vcxproj.filters", "w") as F:
    print(bom + '<?xml version="1.0" encoding="utf-8"?>', file=F)
    ...
    for Filter in P_astra["filter_names"]:
      ...
      for f in l:
        print('    <CudaCompile Include="' + f + '">', file=F)
        print(f'      <Filter>{Filter}</Filter>', file=F)
        print('    </CudaCompile>', file=F)
    print('  </ItemGroup>', file=F)
    print('  <ItemGroup>', file=F)
    for Filter in P_astra["filter_names"]:
  ...
  with open(P["file09"], "w") as F:
    ...
    for Filter in P["filter_names"]:
      ...
      for f in P["filters"][Filter][1:]:
        ...
        print('\t\t\t\t\t>', file=F)
        if ((("Cuda" in f) or ("cuda" in f))) and f[-2:] != ".h":
          for c in configs:
            if not c.cuda:
